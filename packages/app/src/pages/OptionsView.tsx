...
import {
    ...
    Tr,
    useColorModeValue,
    useRadioGroup, useToast, Tooltip, Link
                ...
                tempUserOrderRows.push(
                    <Tr key={id}>
                        ...
                        <Td>
                            {
                                status === 'active' &&
                                <Button colorScheme={bg} size="xs"
                                        onClick={() => cancelOrderFunc(positionHash, userAccount, blockNumber)}>
                                    Cancel
                                </Button>
                            }
                        </Td>
    ...
    useEffect(() => {
        async function main() {
            ...
            while (cursor < latestBlockNumber) {
                ...
                const buyMatches = await getMatchEvents({
                    buyerAddress: account,
                    provider: library,
                    fromBlock: cursor,
                    toBlock: to
                })
                const sellMatches = await getMatchEvents({
