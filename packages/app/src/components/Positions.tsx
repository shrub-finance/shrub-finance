...
function Positions() {
  ...
  const [expiredOptionsRows, setExpiredOptionsRows] = useState<JSX.Element[]>([
    <Tr key={"defaultExpiredOptionRow"} />,
  ]);
  ...
  useEffect(() => {
    const queryBlock =
      shrubfolioData &&
      shrubfolioData._meta &&
      shrubfolioData._meta.block &&
      shrubfolioData._meta.block.number;
    let txBlock = 0;
  ...
  useEffect(() => {
    ...
    if (
      !shrubfolioData ||
      !shrubfolioData.user ||
      !shrubfolioData.user.activeUserOptions
    ) {
      return;
    }
    ...
    for (const userOption of shrubfolioData.user.activeUserOptions) {
      ...
      const pair = `${
        quoteAssetSymbol === "SMATIC" ? "sMATIC" : "sUSD"
      } ${optionType}`;
      const pair2 = `${Number(strike).toLocaleString(undefined, {
        style: "currency",
        currency: "USD",
      })} ${expiry}`;
      ...
      const inTheMoney =
        (optionType === "CALL" && Number(maticPrice) > Number(strike)) ||
      ...
      if (!isExpired) {
        optionRow.push(
          <Tr key={optionId}>
            <Td fontWeight={"medium"} fontSize={"xs"}>
              ...
              <Box>
                ...
                >
                  <Tag
                    size="sm"
                    colorScheme={inTheMoney ? "cyan" : "yellow"}
                    borderRadius="full"
                  >
            ...
            {!isMobile && (
              <Td>
                {orderStack.lastPrice.toLocaleString(undefined, {
                  style: "currency",
                  currency: "USD",
                })}
              </Td>
              ...
              <Box>
                <StatHelpText>
                  {!!orderStack.totalUnrealizedGain && (
                    <StatArrow
                      type={
                        orderStack.totalUnrealizedGain > 0
                          ? "increase"
                          : "decrease"
                      }
                    />
                  )}
                  {orderStack.totalUnrealizedGain.toLocaleString(undefined, {
                    style: "currency",
                    currency: "USD",
                  })}
                </StatHelpText>
  ...
  useEffect(() => {
    ...
    for (const currency of SHRUB_CURRENCIES) {
      ...
      const fluidFontAsset =
        balanceSize > 9
          ? ["md", "2xl", "3xl", "3xl"]
          : ["2xl", "2xl", "3xl", "4xl"];
      const fluidFontSplit =
        balanceSize > 9 ? ["xs", "sm", "lg", "md"] : ["sm", "sm", "lg", "lg"];
      const fluidWidthAsset =
        balanceSize > 9 ? [170, 225, 300, 300] : [200, 270, 300, 370];
      const fluidWidthSplit =
        balanceSize > 9
          ? { sm: "300", md: "300", lg: "300", xl: "200" }
          : "auto";
      ...
      tempShrubfolioRows.push(
        <div key={currency}>
          <Flex align="center" justify="space-evenly">
            ...
            >
              {totalUserBalance(currency)}{" "}
              {currency === "SMATIC" ? "sMATIC" : "sUSD"}
            </Box>

            <Box
              fontSize={fluidFontSplit}
              minW={fluidWidthSplit}
              py={fluidPaddingSplitY}
              ...
              <Box pb={2} color="gray.500" fontWeight="medium">
                ...
                {!isMobile && (
                  <Popover trigger={"hover"}>
                    <PopoverTrigger>
                      <Text ml="1" as="sup" cursor="pointer">
                  ...
                  <Popover trigger={"hover"}>
                    ...
                    <PopoverContent>
                      ...
                      <PopoverBody letterSpacing="wide">
                        <Text>
                          {" "}
                          This amount is available for you to spend or withdraw
                        </Text>
                      </PopoverBody>
                    </PopoverContent>
                  </Popover>
  ...
  function totalUserBalance(currency: string) {
    const totBalance =
      shrubBalance.locked[currency] + shrubBalance.available[currency];
    if (totBalance) {
      return Number(totBalance).toLocaleString(undefined, {
        minimumFractionDigits: ["MATIC"].includes(currency) ? 6 : 2,
      });
    } else return "--";
  }
  ...
  const optionRowTableBg = useColorModeValue(
  ...
  return (
    <>
        ...
        {localError && (
          <>
            <SlideFade in={true} unmountOnExit={true}>
              <Flex>
                <Alert
                  status={web3Error ? "error" : "warning"}
                  borderRadius={"2xl"}
                >
                  <AlertIcon />
      ...
      <Container mt={50} flex="1" borderRadius="2xl" maxW="container.md">
        <Center>
          ...
          <Button
            ...
            onClick={handleWithdrawDepositModalOpen("Withdraw")}
            isDisabled={!active}
          >
            Withdraw
          </Button>
      ...
      <Heading mt={10}>
        <Center>
          <Icon as={BiPaperPlane} mr={2} />
          Option Positions
        </Center>
      </Heading>
      <Container
        mt={50}
        p={hasOptions.current ? 0 : 0}
        flex="1"
        borderRadius="2xl"
        bg={useColorModeValue("white", "dark.100")}
        shadow={useColorModeValue("2xl", "2xl")}
      ...
      >
        {!shrubfolioLoading && !shrubfolioError && account ? (
          ...
          shrubfolioData.user &&
          shrubfolioData.user.activeUserOptions &&
          shrubfolioData.user.activeUserOptions[0] ? (
            <>
              {optionsRows.length ? (
                <Table
                  variant="simple"
                  size="lg"
                  borderTopRadius={expiredOptionsRows.length && "2xl"}
                >
                  <Thead>
                    <Tr>
              ...
              {expiredOptionsRows.length ? (
                ...
                >
                  ...
                  <Tbody bg={expiredRowTableBg}>{expiredOptionsRows}</Tbody>
                </Table>
              ) : null}
            </>
          ) : (
          ...
          <Table variant="simple" size="lg">
            <Thead>
              <Tr>
                ...
                {/*<Th color={"gray.400"}>Balance</Th>*/}
                <Th color={"gray.400"}>Qty</Th>
                <Th color={"gray.400"} display={{ base: "none", md: "flex" }}>
                  Price
                </Th>
                <Th color={"gray.400"}>Gain/Loss</Th>
              </Tr>
            </Thead>
            <Tbody>{spinnerRow}</Tbody>
          </Table>
        )}
      </Container>
                      ...
                      <Flex>
                        ...
                        >
                          ...
                          {withdrawDepositAction === "Deposit"
                            ? walletTokenBalance
                            : Number(
                                shrubBalance.available[modalCurrency]
