...
function OptionDetails({
  ...
  const errorSuggestionColor = useColorModeValue(
    "blackAlpha.700",
    "whiteAlpha.700"
  );
  ...
  useEffect(() => {
    ...
    const sellOrdersBook = sellOrders
      .map((order: any) => {
        return {
          unitPrice: Number(order.pricePerContract),
          formattedSize: order.size,
    ...
    const sellOrdersDepth = sellOrdersBook.reduce(
      (tot: BigNumber, order: any) =>
        tot.add(ethers.utils.parseUnits(order.formattedSize, 6)),
      Zero
    );
    const buyOrdersDepth = buyOrdersBook.reduce(
      (tot: BigNumber, order: any) =>
        tot.add(ethers.utils.parseUnits(order.formattedSize, 6)),
      Zero
    );
    setOrderBook({
      sellOrders: sellOrdersBook,
      buyOrders: buyOrdersBook,
      sellOrdersDepth,
      buyOrdersDepth,
      initialized: true,
    });
  ...
  useEffect(() => {
    ...
    if (!price && !touched) {
      setPrice(
        radioOption === "BUY"
          ? orderBook.sellOrders[0]?.unitPrice.toFixed(4)
          : orderBook.buyOrders[0]?.unitPrice.toFixed(4)
      );
    }
  ...
  useEffect(() => {
    ...
    async function main() {
      ...
      const optionPosition = await userOptionPosition(
        account,
        positionHash,
        library
      );
  ...
  function costForAmount(costAmount: number, optionType: SellBuy) {
    ...
    const orders =
      optionType === "BUY" ? orderBook.sellOrders : orderBook.buyOrders;
    if (
      bigAmount.gt(
        optionType === "BUY"
          ? orderBook.sellOrdersDepth
          : orderBook.buyOrdersDepth
      )
    ) {
      return;
    }
    for (const order of orders) {
      ...
      if (bigSize.gt(remainingSize)) {
        ...
        accumulatedPrice = accumulatedPrice.add(
          bigUnitPrice.mul(remainingSize).div(1e6)
        );
        remainingSize = remainingSize.sub(remainingSize);
        break;
      }
      accumulatedPrice = accumulatedPrice.add(
        bigUnitPrice.mul(bigSize).div(1e6)
      );
      ...
      <>
        {noOrders && radioOrderType === "Market" && (
          <Text
            ...
            pb="2"
          >
            <WarningTwoIcon pr="1" boxSize="3.5" />
            There are no orders in the order book{" "}
          </Text>
        )}
            ...
            </Text>
            {
              ...
              radioOption === "SELL" && optionType === "CALL" ? (
                ...
                >
                  ...
                  {balances &&
                    Number(
                      ethers.utils.formatUnits(balances.shrub.quoteAsset, 18)
                    ).toFixed(4)}{" "}
                  sMATIC
                </Text>
              ) : (
                ...
                >
                  ...
                  {balances &&
                    Number(
                      ethers.utils.formatUnits(balances.shrub.baseAsset, 18)
                    ).toFixed(4)}
                </Text>
              )
            }
        ...
        {insufficientDepth && radioOrderType === "Market" && (
          <Box>
            ...
            >
              <WarningTwoIcon pr="1" boxSize="3.5" />
              Order too large for order book depth{" "}
            </Text>
            <Text
              as={"span"}
              fontWeight="bold"
            ...
            >
              Max:{" "}
              {radioOption === "BUY"
                ? ethers.utils.formatUnits(orderBook.sellOrdersDepth, 6)
                : ethers.utils.formatUnits(orderBook.buyOrdersDepth, 6)}
            </Text>
          </Box>
        )}
      </>
    ...
    try {
      ...
      const unsignedOrder: UnsignedOrder = {
        ...
        price: ethers.utils.parseUnits(
          (Number(price) * Number(newAmount)).toString(),
          18
        ),
      ...
      const verifiedAddress = await getAddressFromSignedOrder(
        signedOrder,
        library
      );
      ...
      const description = `Submitted limit order to ${radioOption.toLowerCase()} ${newAmount} ${formatDate(
        expiry
      )} $${formattedStrike} ${quoteSymbol} ${optionType.toLowerCase()} options for $${price}`;
      const orderToName = {
      ...
      try {
        ...
        pendingTxsDispatch({
          ...
          status: "confirmed",
          data: { blockNumber: receipt.blockNumber, status: "active" },
        });
      } catch (e: any) {
        const toastDescription = ToastDescription(
          description,
  ...
  async function marketOrderMany() {
    try {
      ...
      const localOrderBook =
        radioOption === "BUY" ? orderBook.sellOrders : orderBook.buyOrders;
      // const depth = localOrderBook.reduce((tot, order) => tot.add(order.size), Zero)
      const depth = localOrderBook.reduce(
        (tot, order) =>
          tot.add(ethers.utils.parseUnits(order.formattedSize, 18)),
        Zero
      );
      ...
      while (remainingSize.gt(Zero)) {
        ...
        if (orders.length === 1) {
          ...
          order = orders.find((o) => {
            return (
              ...
              Number(o.formattedSize).toFixed(4) ===
                Number(formattedSize).toFixed(4) &&
        ...
        const doesAddressMatch: boolean = await validateOrderAddress(
          counterPartyOrder,
          library
        );
        ...
        if (orderNonce !== counterpartyNonce) {
          console.error(
            "order nonce does not match the signer of the order's nonce"
          );
          index++;
          continue;
        }
        ...
        if (remainingSize.lt(Zero)) {
          accumulatedPrice = accumulatedPrice.add(
            remainingSize
              .mul(counterPartyOrder.price)
              .div(counterPartyOrder.size)
          );
          remainingSize = Zero;
        }
        ...
        temporaryWorkaroundPrice = bigSize
          .mul(counterPartyOrder.price)
          .div(counterPartyOrder.size);
        index++;
      }
      ...
      const quoteSymbol = await getSymbolFor(quoteAsset, library);
      const description = `${
        radioOption === "BUY" ? "Buy" : "Sell"
      } ${newAmount} ${formatDate(
        expiry
      )} $${formattedStrike} ${quoteSymbol} ${optionType.toLowerCase()} options for $${ethers.utils.formatUnits(
        accumulatedPrice,
        18
      )}`;
      const orderToName = {
      ...
      const pricePerContract =
        Number(ethers.utils.formatUnits(accumulatedPrice)) / Number(newAmount);
      ...
      try {
        ...
        pendingTxsDispatch({
          ...
          txHash: receipt.transactionHash,
          status: "confirmed",
          data: { blockNumber: receipt.blockNumber, status: "completed" },
        });
      } catch (e: any) {
        const toastDescription = ToastDescription(
          description,
  ...
  const formattedPricePerContract = Number(
    ethers.utils.formatUnits(pricePerContract)
  ).toFixed(4);
  const formattedTotPrice =
    totPrice && Number(ethers.utils.formatUnits(totPrice)).toFixed(4);
  const collateralToUnlock =
    balances &&
    Math.min(
      Number(newAmount),
      Number(ethers.utils.formatUnits(balances.optionPosition.abs(), 18))
    ) * (optionType === "CALL" ? 1 : Number(formattedStrike));
  const collateralPerContract =
    optionType === "CALL" ? 1 : Number(formattedStrike);
  const collateralRequirement =
    balances && balances.optionPosition.gt(0)
      ? Math.max(
          0,
          collateralPerContract *
            (Number(newAmount) -
              Number(ethers.utils.formatUnits(balances.optionPosition)))
        )
  ...
  const insufficientFunds =
    ...
    balances &&
    totPrice &&
    balances.shrub.baseAsset.lt(totPrice);
  ...
  const insufficientDepth =
    ...
    (radioOption === "BUY"
      ? bigAmount.gt(orderBook.sellOrdersDepth)
      : bigAmount.gt(orderBook.buyOrdersDepth));
  const noOrders =
    orderBook.initialized &&
    (radioOption === "BUY"
      ? orderBook.sellOrdersDepth.eq(Zero)
      : orderBook.buyOrdersDepth.eq(Zero));
  ...
  return (
    <>
      ...
      <Tabs
        ...
        isLazy
        onChange={(index) =>
          setRadioOrderType(index === 0 ? "Market" : "Limit")
        }
      >
                      ...
                      <Box>
                        ...
                        <Box fontSize={"xs"} fontWeight={"bold"} pb={10}>
                          <Flex>
                            ...
                            {balances && !balances.optionPosition.eq(0) && (
                              <Box>
                                <Box
                                  sx={{ userSelect: "none" }}
                                  borderRadius={"md"}
                      ...
                      <Flex justifyContent={"flex-end"}>
                        {!isMobile && (
                          <Button
                            ...
                            size="xs"
                            colorScheme={"yellow"}
                            onClick={setOrdersVisible.toggle}
                          >
                            View Order Book
                          </Button>
                        )}
                      ...
                      <Box fontSize="sm" pt={6}>
                        <HStack spacing={8} fontSize={"sm"}>
                          ...
                          <VStack
                            ...
                            fontWeight={"600"}
                          >
                            <Text>${formattedPricePerContract}</Text>
                            <Text
                              color={
                                insufficientFunds ? quantityErrorColor : "null"
                              }
                            >
                              ${formattedTotPrice}
                            </Text>
                      ...
                      <Box>
                        ...
                        <NumberInput
                          ...
                          onChange={(valueString) => {
                            ...
                            const numberedValueString = Number(
                              parse(valueString)
                            );
                            ...
                            if (
                              numberedValueString !==
                              Math.round(numberedValueString * 1e6) / 1e6
                            ) {
                        ...
                        <HStack spacing={8} fontSize={"sm"}>
                          <VStack spacing={1.5} alignItems={"flex-start"}>
                            ...
                            {radioOption === "BUY" &&
                              balances &&
                              balances.optionPosition.lt(0) && (
                                <Text>Collateral to unlock</Text>
                              )}
                            {radioOption === "SELL" && (
                              <Text>Collateral Needed</Text>
                            )}
      ...
      >
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            {radioOption === "BUY" ? "Buy" : "Sell"} Order Confirmation
          </AlertDialogHeader>
          <AlertDialogCloseButton />
          <Divider />
          <AlertDialogBody>
