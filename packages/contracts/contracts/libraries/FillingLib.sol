...
library FillingLib {
  ...
  function getAdjustedPriceAndFillSize(OrderLib.SmallOrder memory sellOrder, OrderLib.SmallOrder memory buyOrder) internal view returns (uint, uint) {
  ...
  function fillCallOption(AppStateLib.AppState storage self, address buyer, address seller, OrderLib.OrderCommon memory common, uint fillSize, uint adjustedPrice, bytes32 positionHash) internal {
    ...
    console.log(FundsLib.getAvailableBalance(self, seller, common.quoteAsset));
