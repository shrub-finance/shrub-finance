...
describe("PaperSeed", () => {
  ...
  it("owner should be able to set the baseURI and this should update the tokenURI", async () => {
    const mint = await paperSeed.claimReserve(0);
    const tokenIndex = await mint.wait();
    const oldTokenUri = await paperSeed.tokenURI(0);
    expect(oldTokenUri).to.equal("https://shrub.finance/0");
    const tx = await paperSeed.setBaseURI("https://shrub.finance.new/");
    await tx.wait();
    const newTokenUri = await paperSeed.tokenURI(0);
    expect(newTokenUri).to.equal("https://shrub.finance.new/0");
  });

  it("non-owner should not be able to set the baseURI", async () => {
    const mint = await paperSeed.claimReserve(0);
    const tokenIndex = await mint.wait();
    const oldTokenUri = await paperSeed.tokenURI(0);
    expect(oldTokenUri).to.equal("https://shrub.finance/0");
    const signer1PaperSeed = paperSeed.connect(signer1);
    await expect(
      signer1PaperSeed.setBaseURI("https://shrub.finance.new/")
    ).to.be.revertedWith("Ownable: caller is not the owner");
    const newTokenUri = await paperSeed.tokenURI(0);
    expect(newTokenUri).to.equal("https://shrub.finance/0");
  });
