...
export type GetOrdersParams = Modify<
  Partial<OrderCommon>,
  {
    strike?: string;
  }
...
export type IOrder = UnsignedOrder &
  Signature & {
    address: string;
  };

export type ApiOrder = Modify<
  IOrder,
  {
...
export type AppOrder = AppCommon &
  AppSmall & {
    address?: string;
    size: BigNumber;
...
export type UserOrdersState = {
  [transactionHash: string]: IndexedAppOrderSignedNumbered;
};

export type UserOrdersAction = {
  type: "add";
  order: IndexedAppOrderSignedNumbered;
};
